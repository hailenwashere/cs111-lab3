...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	// lock here so each thread sees an updated version of the hashtable
	pthread_mutex_lock(&(hash_table->lock));

	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	struct list_head *list_head = &hash_table_entry->list_head; // locate head
	struct list_entry *list_entry = get_list_entry(hash_table, key, list_head); // getlist entry
